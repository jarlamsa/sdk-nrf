...
* nRF5340 SoC - Supporting Bluetooth® Low Energy, 802.14.5 based protocols and Near Field Communication (NFC).
...
1. Go to the `Thingy:53 product page`_ and under the :guilabel:`Downloads` tab, navigate to :guilabel:`Precompiled applications and modem firmware`.
...
When you have the precompiled images ready, you can directly program the images onto the Thingy:53 using the nRF Connect Programmer app available in nRF Connect for Desktop.

In this method, the Thingy:53 is connected directly to your PC through USB.
This method makes use of the :doc:`mcuboot:index` feature and the built-in serial recovery mode of Thingy:53.
...
The build targets of interest for Thingy:53 in |NCS| are as follows:
...
+---------------------------------+--------------------------------------+--------------------------------------------------------------------+
| :file:`app_update.bin`          | MCUboot compatible image, HEX format | Using the built-in bootloader and the mcumgr command line tool     |
+---------------------------------+--------------------------------------+--------------------------------------------------------------------+
| :file:`net_core_app_update.bin` | MCUboot compatible image, HEX format | Using the built-in bootloader and the mcumgr command line tool     |
+---------------------------------+--------------------------------------+--------------------------------------------------------------------+
...
#. Build the sample or application:
   ...
   The output of the build with the merged HEX file containing both the application and the SPM is located in the :file:`zephyr` subfolder in the build directory.
...
#. Build the sample or application code as follows:
   ...
   For *build_target*, use:

   * :file:`thingy53_nrf5340_cpuapp` or :file:`thingy53_nrf5340_cpuapp_ns` when building samples for the application core.
   * :file:`thingy53_nrf5340_cpunet` when building for the network core.
